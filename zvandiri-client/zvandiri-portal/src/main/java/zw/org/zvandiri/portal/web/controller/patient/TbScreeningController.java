...
public class TbScreeningController extends BaseController {
    ...
    public String setUpModel(ModelMap map, TbScreening item) {
        map.addAttribute("pageTitle", APP_PREFIX + " " + item.getPerson().getNameOfClient() + "'s Tb Screening History");
    ...
    public String getForm(ModelMap map, @RequestParam(required = false) String id, @RequestParam(required = false) String personId) {
        TbScreening item;
        if (id != null) {
            item = service.get(id);
            return setUpModel(map, item);
        }
        item = new TbScreening(personService.get(personId));
        return setUpModel(map, item);
    }
    ...
    public String getItemList(@RequestParam String id, @RequestParam(required = false) Integer type, ModelMap model) {
        Person item = personService.get(id);
        model.addAttribute("pageTitle", APP_PREFIX + " " + item.getNameOfClient() + "'s Tb Screening History");
        model.addAttribute("person", item);
        if (type != null) {
            model.addAttribute("message", AppMessage.getMessage(type));
        }
        model.addAttribute("items", service.getByPerson(item));
    ...
    public String delete(@Valid ItemDeleteDTO dto) {
        ...
        service.delete(item);
        return "redirect:item.list?type=2&id=" + person.getId();
    }
}
